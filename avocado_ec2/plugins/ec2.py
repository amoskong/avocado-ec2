...
class EC2TestRunner(RemoteTestRunner):
    ...
    def _install_avocado(self, distro_type):
        ...
        if distro_type == 'fedora':
            ...
            install_cmd = 'sudo yum install epel-release -y; sudo yum install -y gcc openssl-devel python-devel python2-pip libvirt libvirt-devel pkgconfig gdb-gdbserver git && sudo pip install -r https://raw.githubusercontent.com/amoskong/avocado-ec2/branch-scylladb/avocado-requirements.txt && sudo pip install avocado-framework==36.4'
        ...
        self.remote.run(retrieve_cmd, timeout=300, quiet=False)
    ...
    def uptime(self):
        ret = None
        try:
            ret = self.remote.uptime()
            self.job.log.debug('uptime response: %s' % ret)
        except:
            pass
        return ret
    ...
    def setup(self):
        try:
            ...
            wait.wait_for(self.uptime, self.job.args.ec2_login_timeout, text='Waiting for SSH to be up')
            self.job.log.debug('start install avocado')
